...
from optimi.utils import debias, debias_beta, device_guard
...
def radam(
    ...
    *,
    ...
    if gradient_release:
        if triton:
            func = _single_param_triton_radam
        elif foreach:
...
if SUPPORTS_TRITON:
    ...
    def _radam_kernel(
        ...
        weight_decay,
        eps,
        rect,  # scalar rectification term (0 if inactive)
        do_rect: tl.constexpr,  # whether to apply rectification (rect is not None)
        ...
        if update_parameters:
            # Compute RAdam step or momentum SGD fallback when unrectified
            if do_rect:
            ...
            if kahan_sum:
                ...
                # Optionally downcast to param.dtype from fp32 intermediates
